```python
   ...
   app = Flask(__name__)
   app.config["JWT_SECRET_KEY"] = os.getenv("JWT_SECRET_KEY", str(uuid.uuid4()))
...
**If Fails**:
- Share `docker logs billy_assistant` output.
- Verify `app/server.py` matches the above.

**iOS**:
- Use `http://192.168.1.112:5001` with ATS exception in `Info.plist`.
- Share iOS code for help.

**Note**: Fixed `flask-limiter` initialization for compatibility. Your setup (Ollama at `192.168.1.112:11434`, `app/server.py`) is noted. Let me know results!